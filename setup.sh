...
REG_EX_STRING = "^force://.*\.salesforce\.com$"
...
echo "value coming for url===> $devHubSfdxAuthUrl"
...
if [[ $devSfdxAuthUrl =~ $REG_EX_STRING ]]; then
...
if [[ $stagingSfdxAuthUrl =~ $REG_EX_STRING ]]; then
...
if [[ $prodSfdxAuthUrl =~ $REG_EX_STRING ]]; then
